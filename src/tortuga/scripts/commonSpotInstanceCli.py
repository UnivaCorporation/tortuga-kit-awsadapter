...
class CommonSpotInstanceCLI(TortugaCli, SpotInstanceCommonMixin):
    def __init__(self):
        super().__init__()

        self.__metadataWsApi: Optional[MetadataWsApi] = None
    ...
    def _get_adapter_cfg(self, name: str) -> dict:
        ...
        if name != 'Default':
            resource_adapter_cfg = resourceAdaterConfigurationWsApi.get(
                'AWS',
                name,
            )['configuration']
    ...
    def __get_adapter_cfg_as_dict(self, adapter_cfg: List[dict]) \
            -> Dict[str, str]:
        """
        Convert list of dict output from resource adapter WS API to
        dict keyed on setting name.
        """
